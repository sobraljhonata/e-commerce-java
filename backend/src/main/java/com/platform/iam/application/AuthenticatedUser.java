 ...
 *
 * <p><strong>Papéis ({@code roles}):</strong> espelham a claim {@code roles} do JWT emitido pelo IAM
 * — essa claim é a <strong>fonte de verdade</strong> para este modelo. As {@code GrantedAuthority}
 * derivadas pelo Spring servem à autorização HTTP; não duplicar regras assumindo equivalência sem
 * revisar o emissor do token.
 *
 ...
 * @param tenantId claim obrigatória {@code tenantId} no JWT (contexto atual; fonte de verdade no token)
